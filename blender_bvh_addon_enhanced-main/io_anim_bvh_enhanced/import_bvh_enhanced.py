...
class BVH_Node:
    ...
    def __init__(self, name, rest_head_world, rest_head_local, parent, channels, rot_order, index):
        ...
        self.has_loc = channels[0] != - \
            1 or channels[1] != -1 or channels[2] != -1
        self.has_rot = channels[3] != - \
            1 or channels[4] != -1 or channels[5] != -1
    ...
    for bvh_node in bvh_nodes_list:
        ...
        if not bvh_node.rest_tail_world:
            if len(bvh_node.children) == 0:
                ...
                bvh_node.rest_tail_world = Vector(
                    bvh_node.children[0].rest_head_world)
                bvh_node.rest_tail_local = bvh_node.rest_head_local + \
                    bvh_node.children[0].rest_head_local
            else:
                ...
                bvh_node.rest_tail_world = rest_tail_world * \
                    (1.0 / len(bvh_node.children))
                bvh_node.rest_tail_local = rest_tail_local * \
                    (1.0 / len(bvh_node.children))
...
def bvh_node_dict2objects(
    ...
    if apply_axis_conversion and global_matrix:
    ...
    if skip_frames >= len(bvh_nodes)-1:
        skip_frames = 0

    if not add_rest_pose_as_first_frame:
        skip_frames += 1 # skip the first data in bvh_node.anim_data = [(0, 0, 0, 0, 0, 0)] added by read_bvh()
    ...
    bvh_nodes_list = sorted_nodes(bvh_nodes)
    root_bone_name = bvh_nodes_list[0].name
       ...
       # Add tail objects
        for name, bvh_node in bvh_nodes.items():
            if not bvh_node.children:
                ...
                ob_end.location = global_matrix_3x3 @ (bvh_node.rest_tail_world - 
                                    bvh_node.rest_head_world)
        ...
        for name, bvh_node in bvh_nodes.items():
            ...
            for frame_current in range(len(bvh_node.anim_data) - skip_frames):
                # print('---> Frame: #', frame_current)
                lx, ly, lz, rx, ry, rz = bvh_node.anim_data[frame_current + skip_frames]
                ...
                if bvh_node.has_rot:
                    ...
                    rot_matrix = euler_orig.to_matrix().to_4x4()
                    rot_matrix = global_matrix @ rot_matrix @ global_matrix_inv
                    euler_new = rot_matrix.to_euler(obj.rotation_mode, prev_euler)
                    prev_euler = euler_new

                    obj.delta_rotation_euler = (euler_new[0], euler_new[1], euler_new[2])
...
def bvh_node_dict2armature(
    ...
    if num_frame > skip_frames:
        num_frame = num_frame - skip_frames
    ...
    for i, bvh_node in enumerate(bvh_nodes_list):
        ...
        # if bvh_node.has_loc:
            ...
            for frame_i in range(num_frame):
                bvh_loc = bvh_node.anim_data[frame_i + skip_frames][:3]

                # print('---> bvh_loc before: ', bvh_loc)
        ...
        if bvh_node.has_rot:
            ...
            prev_euler = Euler((0.0, 0.0, 0.0))
            for frame_i in range(num_frame):
                bvh_rot = bvh_node.anim_data[frame_i + skip_frames][3:]
    ...
    if apply_axis_conversion and global_matrix:
        # print('===> finally apply global_matrix: \n', global_matrix)
        # print('\n'*3)
...
def bvh_node_dict2existing_armature(
        context,
        armature_name,
        bvh_name,
        bvh_nodes,
    ...
    # arm_data = bpy.data.armatures.new(bvh_name)
    # arm_ob = bpy.data.objects.new(bvh_name, arm_data)
    arm_ob = bpy.data.objects[armature_name]
    ...
    if rotate_mode == 'NATIVE':
        # for bvh_node in bvh_nodes_list:
        for bone_name, bvh_node in bvh_nodes.items():
    ...
    # skip_frames = 1
    if skip_frames < 0:
        skip_frames = 0 # don't skip any frames, keep the same frames as in the .bvh file
    if num_frame > skip_frames:
        num_frame = num_frame - skip_frames
    ...
    for i, bvh_node in enumerate(bvh_nodes_list):
        ...
        if bvh_node.has_loc and not skip_loc:
            ...
            for frame_i in range(num_frame):
                bvh_loc = bvh_node.anim_data[frame_i + skip_frames][:3]
                # print('---> bvh_loc before:  \n', bvh_loc)

                bone_translate_matrix = Matrix.Translation(
                    Vector(bvh_loc) - bvh_node.rest_head_local)
                # print('---> bone_translate_matrix before: \n', bone_translate_matrix)

                if apply_axis_conversion and global_matrix:
                    # convert translation data into blender's coordinate systems (Z-up/Y-forward/X-right) 
                    bone_translate_matrix = global_matrix @ bone_translate_matrix
                    # print('---> bone_translate_matrix after:  \n', bone_translate_matrix)
...
def load(
    ...
    if update_scene_duration:
        _update_scene_duration(
            context, report, bvh_frame_count, bvh_frame_time, frame_start, use_fps_scale)
    ...
    if target == 'NEW_ARMATURE':
        bvh_node_dict2armature(
            ...
            apply_axis_conversion=apply_axis_conversion,
            skip_frames=skip_frames,
            add_rest_pose_as_first_frame = add_rest_pose_as_first_frame,
        )
    elif target == 'NEW_PARENTED_OBJECTS':
        ...
        bvh_node_dict2existing_armature(
            context, target, bvh_name, bvh_nodes, bvh_frame_time,
            rotate_mode=rotate_mode,
            frame_start=frame_start,
            IMPORT_LOOP=use_cyclic,
            global_matrix=global_matrix,
        ...
        report({'ERROR'}, "Invalid target %r (must be 'NEW_ARMATURE' or name for existing ARMATURE Objects)" % target)
        return {'CANCELLED'}
...
def _update_scene_duration(
    ...
    if use_fps_scale:
        ...
        scaled_frame_count = int(
            ceil(bvh_frame_count * bvh_frame_time * scene_fps))

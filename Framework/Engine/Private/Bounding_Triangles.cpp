...
{
	CCollider::TRIANGLE_DESC CapsuleDesc = *(CCollider::TRIANGLE_DESC*)pArg;
	
	m_OriginalMyDesc = CapsuleDesc;
	m_MyDesc = CapsuleDesc;
...
HRESULT CBounding_Triangles::Render(PrimitiveBatch<VertexPositionColor>* pBatch)
{
	...
	_vector vVertex1 = XMLoadFloat3(&m_MyDesc.vVertex1);
	_vector vVertex2 = XMLoadFloat3(&m_MyDesc.vVertex2);
	_vector vVertex3 = XMLoadFloat3(&m_MyDesc.vVertex3);
...
void CBounding_Triangles::Triangle_Tranform(_fmatrix WorldMatrix)
{
	...
	// 원본의 점들을 로드
	XMVECTOR v1 = XMLoadFloat3(&m_OriginalMyDesc.vVertex1);
	XMVECTOR v2 = XMLoadFloat3(&m_OriginalMyDesc.vVertex2);
	XMVECTOR v3 = XMLoadFloat3(&m_OriginalMyDesc.vVertex3);
	...
	// 변환된 값을 현재 구조체에 저장
	XMStoreFloat3(&m_MyDesc.vVertex1, v1);
	XMStoreFloat3(&m_MyDesc.vVertex2, v2);
	XMStoreFloat3(&m_MyDesc.vVertex3, v3);

	// 중앙 좌표 계산 (vCenter)
	XMVECTOR center = (v1 + v2 + v3) / 3.0f;
	XMStoreFloat3(&m_MyDesc.vCenter, center);
}

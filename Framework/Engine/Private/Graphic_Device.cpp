...
HRESULT CGraphic_Device::Ready_Graphic_Device(const GRAPHIC_DESC& GraphicDesc, ID3D11Device** ppDeviceOut, ID3D11DeviceContext** ppDeviceContextOut)
{
	...
	if (FAILED(Ready_SwapChain(GraphicDesc.hWnd, GraphicDesc.isWindowed, GraphicDesc.iWinSizeX, GraphicDesc.iWinSizeY)))
		return E_FAIL;

	if (FAILED(Ready_BackBufferRenderTargetView()))
		return E_FAIL;

	if (FAILED(Ready_DepthStencilRenderTargetView(GraphicDesc.iWinSizeX, GraphicDesc.iWinSizeY)))
		return E_FAIL;
...
CGraphic_Device * CGraphic_Device::Create(const GRAPHIC_DESC& GraphicDesc, ID3D11Device ** ppDevice, ID3D11DeviceContext ** ppDeviceContextOut)
{
	CGraphic_Device*		pInstance = new CGraphic_Device();

	if (FAILED(pInstance->Ready_Graphic_Device(GraphicDesc, ppDevice, ppDeviceContextOut)))
	{
